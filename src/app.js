...
function initMap() {
	...
	function updateStreamGraph(data) {
		...
		streamGraph.selectAll("path")
			.data(newSeries)
			.transition().attr("d", area).duration(duration)
			.style("fill", d => { return color(d.key); });

				let axis        = streamGraph.append('g').attr('class', 'x axis')
											       .call(xTime.axis = d3.axisBottom(xTime)
											  			                    .ticks(10)
	...
	function tick() {
		...
		// Shift domain
		xTime.domain([now - limit * duration, now]);
